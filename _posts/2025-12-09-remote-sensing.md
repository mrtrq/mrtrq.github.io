---
layout: post
title: Introduction to remote sensing
date: 2025-12-09 06:48:00
description: remote sensing 101
tags: remote sensing 
categories: published-posts
chart:
  plotly: true
featured: true
---

### What is remote sensing?
Remote sensing is a self-explanatory term where someone can get a sense of something remotely, without the need to visit that something.
There are clear examples of things that are part of our daily life that invloves remote sensing.

### What kind of things remote sensing can do?
Remote sensing can be a powerful tool that can be handy in certain situations. Below are two examples of what it can monitor:

{% tabs rs-monitoring %}

{% tab rs-monitoring WQP %}

Can be used throughout different types of rivers, such as urban river rural river areas
{% endtab %}

{% tab rs-monitoring soil-conditions%}

Actual spatial-temporal analysis of soil conditions can be measured throughout the year by using satellite imagery that orbits our planet Earth
{% endtab %}

{% endtabs %}

### Satellite-based remote sensing

There will be multiple wavelengths that a satelleite will give from a single image. Below is the table for Sentinel-2 satellite.

[insert table]

* note that the corresponding band number and wavelength may differ across different satellites